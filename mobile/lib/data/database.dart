...
class AppDatabase extends _$AppDatabase {
  ...
  Stream<List<EcheanceAvecSolde>> watchEcheancesAvecSolde() {
    ...
    final query = select(echeances).join([
    ...
    return query.watch().map((rows) {
      return rows.map((row) {
        ...
        final joursRetard = DateTime.now().difference(echeance.dateEcheance).inDays;
  ...
  Stream<List<PaiementAvecDemande>> watchPaiementsDeLEcheance(
    ...
    return query.watch().asyncMap((liste) async {
      ...
      for (final paiement in liste) {
        final demande = await (select(demandesValidation)
              ..where((d) =>
                  d.paiementClientUuid.equals(paiement.clientUuid) &
                  d.statut.equals('en_attente')))
            .getSingleOrNull();
        resultats.add(PaiementAvecDemande(
            paiement: paiement, demandeEnAttente: demande != null));
      }
      return resultats;
    });
  }
  ...
  Stream<List<DemandeAvecPaiement>> watchDemandesEnAttente() {
    final query = select(demandesValidation).join([
      innerJoin(paiements,
          paiements.clientUuid.equalsExp(demandesValidation.paiementClientUuid)),
    ])
      ..where(demandesValidation.statut.equals('en_attente'))
      ..orderBy([OrderingTerm.desc(demandesValidation.dateDemande)]);

    return query.watch().map((rows) => rows
        .map((row) => DemandeAvecPaiement(
  ...
  Future<void> traiterDemande({
    ...
    final demande = await (select(demandesValidation)
          ..where((d) => d.clientUuid.equals(demandeClientUuid)))
        .getSingle();

    await (update(demandesValidation)
          ..where((d) => d.clientUuid.equals(demandeClientUuid)))
        .write(DemandesValidationCompanion(
